...
function out = core_matlab( sample, mode, tile, xy, z, ref_round, n_chs, n_rounds, ...
    ...
    %
    addpath(fullfile('/home/huzeng_pkuhpc/gpfs3/yly/STATES-matlab-tissue'));
    ...
    if strcmp(p.Results.mode,'local_registration')
        ...
        %for r = 1:size(sdata_t.registeredImages, 5)
        %    local_registered_img_name = fullfile(interm_output_dir, strcat('local_registered_t', num2str(p.Results.subtile), '_round_', num2str(r), '.tif'));
        %    SaveSingleTiff(sdata_t.registeredImages(:,:,:,:,r), local_registered_img_name);
