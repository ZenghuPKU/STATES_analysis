...
library(dplyr)
library(ggrepel)
setwd("/storage/lingyuan2/STATES_analysis/cellline_downstream/03_TE_totalRNA/TEresults")

data <- read.csv("wilcoxon_results_last_four_columns.csv")

genes_to_label_list <- c("ATF4", "SEC61B", "IL27RA")
...
early_plot <- plot_volcano(
  data, 
  "early_adjusted_p_value", 
  "early_log2_fold_change", 
  "TE - early vs Control"
)

print(early_plot)

ggsave("TE_early_final.pdf", early_plot, width = 4, height = 4, bg = "white")